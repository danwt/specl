...
use specl_symbolic::{SpacerProfile, SymbolicConfig, SymbolicMode, SymbolicOutcome, SymbolicResult};
...
fn run_symbolic(
    ...
    let spec = specl_ir::compile(&module)
        .unwrap_or_else(|e| panic!("compile error in {spec_file}: {e}"));
...
fn kind_dekker_ok() {
    let outcome =
        run_symbolic("dekker.specl", &[], SymbolicMode::KInduction(2)).unwrap();
    assert!(
        matches!(outcome, SymbolicOutcome::Ok { .. }),
        "expected Ok, got: {outcome:?}"
    );
