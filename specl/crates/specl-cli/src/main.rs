...
/// Parse, type-check, and compile a specl source file.
fn compile_spec(
    file: &PathBuf,
) -> CliResult<(specl_syntax::Module, specl_ir::CompiledSpec, Arc<String>)> {
    ...
    Ok((module, spec, source))
}

/// Warn if spec has Seq[T] variables that will be bounded in symbolic mode.
fn warn_seq_vars(spec: &specl_ir::CompiledSpec, seq_bound: usize, quiet: bool) {
    let has_seq_vars = spec
        .vars
        .iter()
        .any(|v| matches!(&v.ty, specl_types::Type::Seq(..)));
    if has_seq_vars && !quiet {
        eprintln!(
...
fn main() {
    ...
    let result = match cli.command {
        ...
        Commands::Check { help, verbose, .. } if help => {
        ...
        Commands::Check {
            ...
            let sym_flags = SymbolicFlags {
                ...
                golem,
                seq_bound,
                spacer_profile: parse_spacer_profile(&spacer_profile),
                timeout,
            };

            let bfs_flags = BfsFlags {
                max_states,
                max_depth,
                ...
                profile,
                check_only_invariants: check_only,
                diff_traces: diff,
                swarm,
            };
            ...
            if orig_file.display().to_string().ends_with(".ts.json") {
                let inner = cmd_check_ts(&orig_file, &constant, &sym_flags, &bfs_flags);
                if let Err(e) = inner {
            ...
            let use_symbolic = symbolic || sym_flags.has_specific_technique();
            let use_bfs = bfs || bfs_flags.has_specific_option();
            ...
            let inner = if use_symbolic {
                cmd_check_symbolic(
                    ...
                    &sym_flags,
                    json,
                    bfs_flags.check_only_invariants,
                )
            } else if use_bfs {
                cmd_check(&file, &constant, &bfs_flags)
            } else {
                ...
                if auto_symbolic {
                    ...
                    let sym_result = cmd_check_symbolic(
                        ...
                        &auto_sym_flags,
                        json,
                        bfs_flags.check_only_invariants.clone(),
                    );
...
fn cmd_info(file: &PathBuf, constants: &[String]) -> CliResult<()> {
    let (module, spec, _source) = compile_spec(file)?;
...
/// to BFS or symbolic backend.
fn cmd_check_ts(
    file: &PathBuf,
    constants: &[String],
    sym_flags: &SymbolicFlags,
    bfs_flags: &BfsFlags,
) -> CliResult<()> {
    ...
    let want_symbolic = sym_flags.has_specific_technique();
    let want_bfs = bfs_flags.has_specific_option();
    let json = bfs_flags.output_format != OutputFormat::Text;
    ...
    if want_symbolic {
        return run_symbolic_check_with_spec(
            spec,
            consts,
            sym_flags,
            json,
    ...
    if has_unbounded {
        if !bfs_flags.quiet {
            eprintln!();
            eprintln!(
                "Symbolic checking failed and BFS is not possible (unbounded types like Nat/Int)."
            );
            eprintln!("Try constraining types with range bounds, or fix the symbolic error.");
        }
        return sym_result;
...
fn run_symbolic_check_with_spec(
    ...
    let config = sym_flags.build_config();
    warn_seq_vars(&spec, sym_flags.seq_bound, json || quiet);
    ...
    run_symbolic_check(
        ...
        sym_flags.inductive,
        sym_flags.ic3,
        sym_flags.k_induction,
        sym_flags.bmc_depth,
    )
}

/// BFS check from a .specl file.
...
/// Used by both `cmd_check` (from .specl files) and `cmd_check_ts_bfs` (from .ts.json files).
fn run_bfs_check(
    ...
    if !flags.quiet {
        print_profile(&spec_profile, flags.use_por, flags.use_symmetry);
    }

    // Warn about unbounded types
    let unbounded_warnings: Vec<_> = spec_profile
        .warnings
        .iter()
        .filter(|w| matches!(w, specl_ir::analyze::Warning::UnboundedType { .. }))
        .collect();
    if !unbounded_warnings.is_empty() && !flags.quiet {
    ...
    let resolved = auto_enable_features(
        ...
        flags.fast_check,
        flags.collapse,
        flags.user_has_explicit_storage(),
        flags.no_auto,
        flags.quiet,
    );
    ...
    let mut parallel = flags.parallel;
    if flags.profile && parallel {
        if !flags.quiet {
    ...
    if flags.directed && parallel && !flags.quiet {
        eprintln!("Note: --directed uses priority BFS which is sequential");
    }

    if flags.bloom && !flags.quiet {
    ...
    let spinner = create_spinner(&progress, start, flags.quiet);

    let config = CheckConfig {
        check_deadlock: flags.check_deadlock,
        max_states: flags.max_states,
        max_depth: flags.max_depth,
        memory_limit_mb: flags.memory_limit_mb,
        parallel,
        ...
        bloom: flags.bloom,
        bloom_bits: flags.bloom_bits,
        directed: flags.directed,
        max_time_secs: flags.max_time_secs,
        check_only_invariants: flags.check_only_invariants.clone(),
        collapse: actual_collapse,
        tree: flags.tree,
    };

    // Incremental cache
    let (cache_hash, cached_fps) = match (flags.incremental, &source) {
        (true, Some((src, file))) => {
            let const_pairs = spec
                .consts
                .iter()
                .zip(consts.iter())
                .filter_map(|(c, v)| {
                ...
                .collect::<Vec<_>>();
            let hash = specl_mc::cache::spec_hash(src, &const_pairs);
    ...
    if let (Some(hash), Some((_, file))) = (cache_hash, source) {
    ...
    let exit_code = render_bfs_result(BfsResultContext {
        result,
        secs,
        explored_states,
        observed_max_depth,
...
/// Symbolic check from a .specl file.
fn cmd_check_symbolic(
    ...
    let (module, mut spec, _source) = compile_spec(file)?;
    ...
    let config = sym_flags.build_config();
    warn_seq_vars(&spec, sym_flags.seq_bound, json);
...
fn cmd_simulate(
    ...
    let json = output == OutputFormat::Json;
    let (_module, spec, _source) = compile_spec(file)?;
    let consts = parse_constants(constants, &spec)?;
...
/// Context needed to render BFS check results across all output formats.
struct BfsResultContext<'a> {
    result: CheckOutcome,
    secs: f64,
    ...
    diff_traces: bool,
    actual_por: bool,
    actual_symmetry: bool,
    actual_fast: bool,
    actual_collapse: bool,
    bloom: bool,
    directed: bool,
    explorer: &'a Explorer,
...
fn render_bfs_result(ctx: BfsResultContext<'_>) -> i32 {
    if ctx.output_format == OutputFormat::Dot {
        return render_dot_output(ctx);
    }
    if ctx.output_format == OutputFormat::Itf || ctx.output_format == OutputFormat::Mermaid {
        return render_trace_format_output(ctx);
    }
    let json = ctx.output_format == OutputFormat::Json;
    if json {
        return render_json_output(ctx);
    }
    render_text_output(ctx)
}

fn render_dot_output(ctx: BfsResultContext<'_>) -> i32 {
    let store = ctx.explorer.store();
    ...
    let violation_fp = match &ctx.result {
    ...
    match ctx.result {
        CheckOutcome::InvariantViolation { .. } | CheckOutcome::Deadlock { .. } => 1,
        CheckOutcome::StateLimitReached { .. }
        | CheckOutcome::MemoryLimitReached { .. }
        | CheckOutcome::TimeLimitReached { .. } => 2,
        _ => 0,
    }
}

fn render_trace_format_output(ctx: BfsResultContext<'_>) -> i32 {
    match ctx.result {
        CheckOutcome::InvariantViolation { invariant, trace } => {
            if ctx.output_format == OutputFormat::Mermaid {
                println!(
                    "{}",
                    trace_to_mermaid(
                        &trace,
        ...
        CheckOutcome::Deadlock { trace } => {
            if ctx.output_format == OutputFormat::Mermaid {
                println!(
                    "{}",
                    trace_to_mermaid(&trace, ctx.var_names, "deadlock", None)
                );
            } else {
                let itf = trace_to_itf(&trace, ctx.var_names, "deadlock", None);
                println!("{}", serde_json::to_string_pretty(&itf).unwrap());
            }
            1
        }
...
fn render_json_output(ctx: BfsResultContext<'_>) -> i32 {
    let out =
        match ctx.result {
            CheckOutcome::Ok {
                states_explored,
                max_depth,
            } => JsonOutput::new("ok", ctx.secs).with_states(states_explored, max_depth),
            CheckOutcome::InvariantViolation { invariant, trace } => {
                JsonOutput::new("invariant_violation", ctx.secs)
                    .with_states(ctx.explored_states, ctx.observed_max_depth)
                    .with_invariant(invariant)
                    .with_trace(trace_to_json(&trace, ctx.var_names))
            }
            CheckOutcome::Deadlock { trace } => JsonOutput::new("deadlock", ctx.secs)
                .with_states(ctx.explored_states, ctx.observed_max_depth)
                .with_trace(trace_to_json(&trace, ctx.var_names)),
            CheckOutcome::StateLimitReached {
                states_explored,
                max_depth,
            } => JsonOutput::new("state_limit_reached", ctx.secs)
                .with_states(states_explored, max_depth),
            CheckOutcome::MemoryLimitReached {
                states_explored,
                max_depth,
                memory_mb,
            } => JsonOutput::new("memory_limit_reached", ctx.secs)
                .with_states(states_explored, max_depth)
                .with_memory(memory_mb),
            CheckOutcome::TimeLimitReached {
                states_explored,
                max_depth,
            } => JsonOutput::new("time_limit_reached", ctx.secs)
                .with_states(states_explored, max_depth),
        };
    println!("{}", serde_json::to_string(&out).unwrap());
    match out.result {
...
fn render_text_output(ctx: BfsResultContext<'_>) -> i32 {
    match ctx.result {
        CheckOutcome::Ok {
            states_explored,
            max_depth,
            ...
            if ctx.secs > 0.001 {
                println!(
                    "  Throughput: {} states/s",
                    format_large_number((states_explored as f64 / ctx.secs) as u128)
                );
            }
            ...
            if ctx.actual_symmetry {
                opts.push("symmetry");
            }
            if ctx.bloom {
                opts.push("bloom");
            } else if ctx.actual_fast {
                opts.push("fast");
            }
            if ctx.actual_collapse {
                opts.push("collapse");
            }
            if ctx.directed {
                opts.push("directed");
        ...
        CheckOutcome::InvariantViolation { invariant, trace } => {
            ...
            println!("Result: INVARIANT VIOLATION");
            println!("  Invariant: {}", invariant);
            print_text_trace(&trace, ctx.var_names, ctx.diff_traces);
            1
        }
        CheckOutcome::Deadlock { trace } => {
            println!();
...
/// Bundled symbolic checking flags to avoid passing many individual parameters.
struct SymbolicFlags {
    bmc: bool,
    bmc_depth: usize,
    inductive: bool,
    k_induction: Option<usize>,
...
impl SymbolicFlags {
    fn resolve_mode(&self) -> SymbolicMode {
        if self.portfolio {
            ...
            SymbolicMode::Ic3
        } else if let Some(k) = self.k_induction {
            SymbolicMode::KInduction(k)
        } else if self.inductive {
    ...
    fn build_config(&self) -> SymbolicConfig {
        SymbolicConfig {
    ...
    fn has_specific_technique(&self) -> bool {
        self.bmc
            || self.inductive
            || self.k_induction.is_some()
            || self.ic3
            || self.portfolio
            || self.golem
    }
...
/// Bundled BFS checking flags to avoid passing many individual parameters.
struct BfsFlags {
    max_states: usize,
    max_depth: usize,
    memory_limit_mb: usize,
...
impl BfsFlags {
    fn has_specific_option(&self) -> bool {
        self.use_por
            || self.use_symmetry
            || self.fast_check
            || self.bloom
    ...
    fn user_has_explicit_storage(&self) -> bool {
        self.fast_check || self.bloom || self.collapse || self.tree || self.directed
    }
}
